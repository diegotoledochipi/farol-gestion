...
-- ─────────────────────────────────────────
-- PAGOS A SOCIOS (registrar distribuciones)
-- ─────────────────────────────────────────
create table if not exists pago_socio (
  id        uuid primary key default gen_random_uuid(),
  socio_id  text not null check (socio_id in ('pepe','nahuel','mica_diego')),
  monto     numeric(12,2) not null,
  nota      text,
  fecha     date not null default current_date,
  created_at timestamptz default now()
...
-- Columna unidad en insumo (si no existe)
alter table insumo add column if not exists unidad text default 'unidad';

-- Columna descripcion en producto (si no existe)
alter table producto add column if not exists descripcion text;

-- Estado no_pagado en gasto
alter table gasto drop constraint if exists gasto_estado_pago_check;
alter table gasto add constraint gasto_estado_pago_check
  check (estado_pago in ('pagado','pendiente','no_pagado'));

alter table gasto drop constraint if exists gasto_pagado_por_check;
alter table gasto add constraint gasto_pagado_por_check
  check (pagado_por in ('caja_farol','pepe','nahuel','mica_diego','no_pagado'));
